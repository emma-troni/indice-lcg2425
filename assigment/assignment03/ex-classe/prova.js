...
const continents = {}; 
//quando si confronteranno le length dei fiumi, 0 sarà sicuramente < a qualsiasi lunghezza --> max(maxLength, length) il xprimo valore della lunghezza sostituirà il valore iniziale 0
let maxLength = 0;
...
function gestioneData(data) {
  //il ciclo for itera su ogni riga di data = table = dataset csv --> w3: The JavaScript for of statement loops through the values of an iterable object. It lets you loop over iterable data structures such as Arrays, Strings...
  for (let row of data.rows) {
    ...
    //minLength all'inizio è = Infinity --> lunghezza del primo fiume diventa il nuovo valore salvato in minLength. nelle iterazioni successive minLength è confrontato con le nuove lunghezze dei fiumi e viene aggiornato se viene trovato un valore più piccolo --> alla fine del ciclo, minLength contiene la lunghezza minima di tutti i fiumi nel dataset. stesso ragionamento per le altre variabili
  ...
  //secondo ciclo for che itera di nuovo per tutte le righe del dataset e aggiunge il nome del continente (row.get('continent')) alla variabile creata prima
  //w3school --> Create a Set and use add() to add values
  for (let row of data.rows) {
    continentsSet.add(row.get('continent'));
  }
  
  //ciclo for che itera per ogni elemento del Set
  for (let continent of continentsSet) {
    //obj continents creato in precedenza globalmente e inizialmente lasciato vuoto
  ...
  for (let row of data.rows) {
    let continent = row.getString('continent').trim();
    let length = row.getNum('length');
    ...
    //recap: oggetto che contiene i continenti come chiavi --> ogni chiave ha come valore una array di oggetti (ogni oggetto rappresenta un fiume). l'oggetto-fiume contiene tre valori ossia name, length e temperature che sono stati presi dal file csv
  }
}
...
function drawSketch(titleHeight, sketchHeight) {  
  ...
  //rotation contiene il valore dell'angolo di rotazione, ogni volta che viene chiamata questa riga aumenta di 0.001 rad
  rotation += 0.001;  
  ...
  //ciclo che itera per tutte le chiavi (che sono i continenti) dell'oggetto continents 
  for (let continent in continents) {
    //disegno i settori circolari = continenti
    //continents[continent] accede al valore associato alla chiave continent nell'oggetto continents e restituisce una array composta da tutti gli oggetti/fiumi che appartengono a quel continente/chiave
    let rivers = continents[continent];
    //angolo in rad che occupa ogni settore circ/continente --> rivers.length / riversNumber è proporzione fiumi del continente corrente/tutti i fiumi del dataset
    ...
    for (let i = 0; i < rivers.length; i++) {
      ...
      //startAngle è l'angolo di partenza del settore del continente mentre sectorAngle è l'ampiezza del settore che rappresenta il continente stesso (vedi sopra) --> sectorAngle / rivers.length calcola l'ampiezza per ogni linea all'interno del settore. + 0.5 è un piccolo incremento che mi permette di evitare che la prima linea coincida con il bordo del settore steso
      ...
      textFont("Tahoma"); 
      text(riverName, 0, 0, radius*0.38);
      pop();
    }
...
function drawLegend(legendHeight) {
  ...
  //recupero il numero di continenti dall'oggetto continents poi aggiungo 2 (valori temperatura) + 1 x lunghezza
